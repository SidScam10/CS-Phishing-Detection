...
def create_confusion_matrix(y_test, y_pred, save_plot=True):
    ...
    if save_plot:
        plt.savefig('./ML_Script_Evaluation/confusion_matrix.png', dpi=300, bbox_inches='tight')
...
def create_roc_curve(y_test, y_pred_proba, save_plot=True):
    ...
    if save_plot:
        plt.savefig('./ML_Script_Evaluation/roc_curve.png', dpi=300, bbox_inches='tight')
...
def save_evaluation_report(cv_results, test_results, filename='./ML_Script_Evaluation/model_evaluation_report.txt'):
    """
...
def main():
    ...
    try:
        ...
        print("💡 Make sure to run ml_training.ipynb first to train the model")
        return
